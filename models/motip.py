...
from .GMODet.GMODet import build_rt_detr
...
class MOTIP(nn.Module):
    def __init__(self, config: dict):
        ...
        elif self.detr_framework == "GMODet":
            self.detr, self.detr_criterion, _ = build_rt_detr(detr_args)
        else:
    ...
    def inference(
        ...
        newborn_neg_filter = (
            (torch.tensor(ids).to(current[0].confs.device) == -1)
    ...
    def forward_train(
        ...
        pred_id_words, gt_id_words, loss = self.seq_decoder(
        ...
        return pred_id_words, gt_id_words, loss
...
def pad_trajectory_history(trajectory_history_list: list, target_length: int) -> list:
    """Pad trajectory history list with last element if shorter than target length.
    
    Args:
        trajectory_history_list: List of trajectory history instances.
        target_length: Desired length of the list.
        
    Returns:
        Padded list with last element repeated if needed.
    """
    if len(trajectory_history_list) >= target_length:
        return trajectory_history_list
    
    if not trajectory_history_list:
        raise ValueError("Cannot pad empty trajectory history list")
    
    last_element = trajectory_history_list[-1]
    return trajectory_history_list + [last_element] * (target_length - len(trajectory_history_list))