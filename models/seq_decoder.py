...
from .htfe import *
from .cluster_transformer import ClusteringTransformer
...
class SeqDecoder(nn.Module):
    def __init__(
        ...
        self.Affirm_layer = Affirm(patch_size=1, seq_len=max_temporal_length-1, emb_dim=256, dropout=0.1, depth=2, mask_ratio=0.4, pred_len=max_temporal_length-1)

        self.k_attn = ClusteringTransformer(dim=self.hidden_dim, max_seq_len=self.max_temporal_length-1, heads=self.num_heads, window_size=5, num_clusters=None,
                                       causal=False,attn_dropout=0.1,shared_qk=True)  

        self.tmfm = TemporalMFM(dim=self.hidden_dim, num_features=2, reduction=8)
        return
    ...
    def forward(
        ...
        if self.multi_times_id_decoder == 0 or not self.training:
            ...
            if self.training or inference_ensemble == 0:
                ...
                pred_id_words, gt_id_word, loss = self.net_forward(
        ...
        return pred_id_words, gt_id_word, loss
    ...
    def net_forward(
        ...
        # Adapter
        trajectory_feature_embeds0 = format_seq["trajectory"]["features"]
        unknown_features = format_seq["unknown"]["features"]

        trajectory_feature_embeds1 = self.Affirm_layer(trajectory_feature_embeds0)

        if self.training:
            trajectory_feature_embeds2, loss = self.k_attn(trajectory_feature_embeds0.permute(1,0,2))
        else:
            trajectory_feature_embeds2 = self.k_attn(trajectory_feature_embeds0.permute(1,0,2))
        trajectory_feature_embeds2 = trajectory_feature_embeds2.permute(1,0,2)

        trajectory_feature_embeds = self.tmfm([trajectory_feature_embeds1, trajectory_feature_embeds2])

        trajectory_feature_embeds = trajectory_feature_embeds + trajectory_feature_embeds0
        ...
        if self.training:
            return id_words[None, ...], id_gt_words[None, ...], loss if id_gt_words is not None else None
        else:
...
class TemporalMFM(nn.Module):
    def __init__(self, dim, num_features=2, reduction=8):
        super(TemporalMFM, self).__init__()
        
        self.num_features = num_features
        d = max(int(dim/reduction), 4)
        
        # Temporal average pooling (along T dimension)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(d, dim*num_features, bias=False)
        )
    ...
    def forward(self, in_feats):
        """
        Args:
            in_feats: list of temporal features [N, T, C]
        Returns:
            out: merged feature [N, T, C]
        """
        B, T, C = in_feats[0].shape
        
        # Stack features along new dimension [N, num_features, T, C]
        stacked_feats = torch.stack(in_feats, dim=1)
        
        # Sum across features to get global context [N, T, C]
        feats_sum = torch.sum(stacked_feats, dim=1)
        
        # Compute attention weights [N, num_features, C]
        attn = self.temp_pool(feats_sum.transpose(1, 2))  # [N, C, 1]
        attn = attn.squeeze(-1)  # [N, C]
        attn = self.mlp(attn)  # [N, num_features*C]
        attn = attn.view(B, self.num_features, C)  # [N, num_features, C]
        attn = self.softmax(attn)  # softmax along num_features dim
        
        # Apply attention [N, num_features, T, C] * [N, num_features, 1, C]
        out = torch.sum(stacked_feats * attn.unsqueeze(2), dim=1)
