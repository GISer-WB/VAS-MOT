...
def submit_one_seq(
    ...
    for i, (image, ori_image) in enumerate(seq_dataloader):
        ...
        detr_det_idxs = detr_scores > det_thresh
        detr_det_logits = detr_logits[detr_det_idxs]
        detr_det_labels = torch.max(detr_det_logits, dim=-1).indices + 1
        detr_det_boxes = detr_outputs["pred_boxes"][detr_det_idxs]
        detr_det_outputs = detr_outputs["outputs"][detr_det_idxs]
        area_legal_idxs = (detr_det_boxes[:, 2] * ori_w * detr_det_boxes[:, 3] * ori_h) > area_thresh
        ...
        if only_detr is False:
            ...
            if len(trajectory_history) == 1:
                newborn_filter = (trajectory_history[0].confs > newborn_thresh).reshape(-1, )
                trajectory_history[0] = trajectory_history[0][newborn_filter]
                box_results = box_results[newborn_filter.cpu()]
                detr_det_labels = detr_det_labels[newborn_filter.cpu()]
                ...
                if boxes_keep is not None:
                    ...
                    detr_det_labels = detr_det_labels[boxes_keep.cpu()]
        else:
        ...
        if fake_submit is False:
            ...
            with open(result_file_path, "a") as file:
                ...
                for obj_id, box, label in zip(id_results, box_results, detr_det_labels):
                    obj_id = int(obj_id.item())
                    x1, y1, x2, y2 = box.tolist()
                    if dataset in ["DanceTrack", "MOT17", "SportsMOT", "MOT17_SPLIT", "MOT15", "MOT15_V2", "VAS-MOT"]:
                        result_line = f"{i + 1}," \
                                    f"{obj_id}," \
                                    f"{x1},{y1},{x2 - x1},{y2 - y1},{label},-1,-1,-1\n"
                    else:
...
def get_seq_names(data_root: str, dataset: str, data_split: str):
    if dataset in ["DanceTrack", "SportsMOT", "MOT17", "MOT17_SPLIT", "VAS-MOT"]:
