...
class SeqDataset(Dataset):
    def __init__(self, seq_dir: str, dataset: str, height: int = 800, width: int = 1333):
        ...
        """
        image_paths = sorted(os.listdir(os.path.join(seq_dir, "img")))
        image_paths = [os.path.join(seq_dir, "img", _) for _ in image_paths if ("jpg" in _) or ("png" in _)]
    ...
    def process_image(self, image):
        ...
        # if max(h, w) <= 480:
        #     dim = 480
        # elif 480 < max(h, w) <= 576:
        #     dim = 576
        # elif 576 < max(h, w) <= 672:
        #     dim = 672
        # else:
        #     dim = 768
