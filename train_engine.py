...
def train_one_epoch(config: dict, model: MOTIP, logger: Logger,
    ...
    for i, batch in enumerate(dataloader):
        ...
        if config["TRAIN_STAGE"] == "only_detr":    # only train detr part:
            ...
            pred_id_words, gt_id_words, kmeans_loss = get_model(model).forward_train(
        ...
        if config["TRAIN_STAGE"] == "only_detr":    # only need detr loss:
            ...
            loss = detr_loss + id_loss * id_criterion.weight + kmeans_loss*10
        ...
        metrics["hw_cls_loss"].update(detr_loss_dict["loss_hw"].item())
        metrics["kmeans_loss"].update(kmeans_loss.item()*10)
