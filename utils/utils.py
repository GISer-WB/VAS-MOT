...
def combine_detr_outputs(detr_outputs1, detr_outputs2):
    ...
    combined_outputs["hw_outputs"] = torch.cat([detr_outputs1["hw_outputs"], detr_outputs2["hw_outputs"]], dim=0)
...
def detr_outputs_index_select(detr_outputs, index, dim: int = 0):
    ...
    selected_detr_outputs["hw_outputs"] = torch.index_select(detr_outputs["hw_outputs"], index=index, dim=dim).contiguous() #add
